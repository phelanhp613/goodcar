...
<div class="sellbest container py-4">
    ...
    <div class="row">
        ...
        <div class="col-md-3 mb-3">
            <div class="card">
                <img src="{{ getMainImage($item->images) }}" class="card-img-top w-100" alt="...">
                <div class="card-body">
                    <div class="card-title fw-bold">{{ $item->name }}</div>
                    <div class="min-price">Chỉ từ: <span class="text-success">{{ currency_format($item->variants->first()->price ?? 0) }}</span></div>
...
@foreach($productCategories as $cate)
    @if($productByCategory[$cate->id]->isNotEmpty())
        <div class="brand container py-4">
            <h2 class="mb-4 fs-4 border-bottom">{{ $cate->name }}</h2>
            <div class="row">
                @foreach($productByCategory[$cate->id] as $item)
                <div class="col-md-3 mb-3">
                    <div class="card">
                        <img src="{{ getMainImage($item->images) }}" class="card-img-top w-100" alt="...">
                        <div class="card-body">
