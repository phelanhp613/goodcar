...
(function () {
	var u = function (d, k) {
		...
		var l = /^\s*(\d+)((px)|\%)?\s*$/i, y = /(^\s*(\d+)((px)|\%)?\s*$)|^$/i,
			...
			z = function () {
				...
				if ("true" == c.getCustomData("isReady")) {
					var d = a.getContentElement("info", "txtWidth"),
						h = a.getContentElement("info", "txtHeight"), f;
			...
			}, r = p("btnLockSizes"), x = p("btnResetSize"),
		...
		return {
			...
			onOk: function () {
				if (this.imageEditMode) {
					...
					"image" == k ? this.imageElement = d.document.createElement("img") : (this.imageElement = d.document.createElement("input"),
